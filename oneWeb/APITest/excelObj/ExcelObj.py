...
class ParseExcel(object):
    ...
    def mergeCell(self,sheet,content,coordinate=None,rowNo=None,colsNo=None,endRow=None,endColum=None,style=None):
        if coordinate is not None and ":" in coordinate:
            try:
                sheet.merge_cells(coordinate)
                index = coordinate.split(":")[0]
                sheet[index]=content
                sheet[index].alignment  =  Alignment(horizontal='center', vertical='center')
                if style is not None:
                    sheet[index].font=Font(color=self.RGBDict[style])
                # self.workbook.save(self.excelFile)
            except Exception as e:
                raise e
        elif coordinate==None and rowNo is not None and colsNo is not None and endRow is not None and endColum is not None:
            try:
                sheet.merge_cells(start_row=rowNo, start_column=colsNo, end_row=endRow, end_column=endColum)
                sheet.cell(row=rowNo,column=colsNo).value=content
                if style:
                    sheet.cell(row=rowNo,column=colsNo).font=Font(color=self.RGBDict[style])
                # self.workbook.save(self.excelFile)
